...
public:
    ...
    void throw_server_error_or_response_code(const std::string &answer, Json::Value &j) {
        ...
        std::stringstream jr(answer);

        if (!Json::parseFromStream(builder, jr, &j, &parse_error)) {
    ...
    void make(const std::string &env_id) {
        Json::Value req;
        req["env_id"] = env_id;
        Json::Value ans = POST("/v1/envs/", req.toStyledString());
        instance_id = require(ans, "instance_id");
        if (verbose) printf(" * created %s instance_id=%s\n", env_id.c_str(), instance_id.c_str());
    }

    boost::shared_ptr<Space> action_space() {
        if (!space_act)
            space_act = space_from_json(GET("/v1/envs/" + instance_id + "/action_space"));
        return space_act;
    }

    boost::shared_ptr<Space> observation_space() {
        if (!space_obs)
            space_obs = space_from_json(GET("/v1/envs/" + instance_id + "/observation_space"));
        return space_obs;
    }

    static void observation_parse(const Json::Value &v, std::vector<float> &save_here) {
        if (!v.isArray())
    ...
    void reset(State *save_initial_state_here) {
        Json::Value ans = POST("/v1/envs/" + instance_id + "/reset/", "");
        observation_parse(ans["observation"], save_initial_state_here->observation);
    }

    void step(const std::vector<float> &action, bool render, State *save_state_here) {
        Json::Value act_json;
        ...
        Json::Value ans = POST("/v1/envs/" + instance_id + "/step/", act_json.toStyledString());
    ...
    void monitor_start(const std::string &directory, bool force, bool resume) {
        Json::Value data;
        data["directory"] = directory;
        data["force"] = force;
        data["resume"] = resume;
        POST("/v1/envs/" + instance_id + "/monitor/start/", data.toStyledString());
    }
