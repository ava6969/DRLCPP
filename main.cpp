...
int main()
{
    ...
    FCQ onlineModel(nS, nA, device, true);
    FCQ targetModel(nS, nA, device, true);
   ...
   auto buffer = new ReplayBuffer({ nS }, nA, 50000, 64, device, seed);
    ...
    DQN agent(onlineModel, targetModel, buffer, trainStrategy, evalStrategy,
              device, nWarmUpBatches, updateEvery, std::numeric_limits<float>::max(),true,0.1);
    agent.train(CartPole, optimizer, seed, gamma, 100, maxMinutes, maxEpisode, 495);
